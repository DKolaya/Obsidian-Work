...
- 2026-08-04 — CDH_EL: Phase 3 work (Tasks 3.1-3.5, previously uncommitted) landed via new `Drew/Sprint4/*` branches merged into `Drew/Sprint3/Audit` — 7 commits today. Also added beyond the original plan scope: Add User admin page, editable name fields on User Maintenance, and a docs-only Permission Catalog dynamism design note. A real bug was caught and fixed along the way: the audit-log-permission migration's Down script was double-wrapping each module into `{"value": "...escaped json..."}` via `FOR JSON PATH` instead of restoring the original array shape — fixed with `STRING_AGG` concatenation, verified byte-for-byte match to the pristine original. Task 3.6 (optional caching) still not started. **Needs attention:** several checkbox items in this plan (Task 3.1-3.5 "Commit:" lines) are stale — the work shipped via differently-named branches/commits than this doc anticipated; worth a pass to reconcile or archive this plan doc once Phase 3 is confirmed fully landed on `develop`.
- 2026-08-03 — CDH_EL: no new commits since 2026-07-22, working tree still clean — confirms the 07-23 entry, nothing has moved in 11 days. Monday.com EL Tasks: "Implement audit trail" now shows Waiting for review (was In Progress), consistent with PR #10 having been opened. PR [CDH_EL#10](https://github.com/CDHTS/CDH_EL/pull/10) still CONFLICTING as of 07-27 — needs a rebase before it can merge.
- 2026-07-23 — CDH_EL: 2 more commits landed on `Drew/Sprint3/Audit` (`09092b2` audit log bugfixes, `8b29d1e` perf: cache permission documents to cut per-render DB checks). Working tree clean. PR [CDH_EL#10](https://github.com/CDHTS/CDH_EL/pull/10) ("Audit and permission system") opened for this branch — currently showing as CONFLICTING, needs a rebase before it can merge. Monday.com EL Tasks board reachable again (outage from 2026-07-20/22 resolved); Drew's 3 in-progress items (audit trail, notes, application permissions) unchanged since 2026-07-20.
- 2026-07-22 — CDH_EL: Phase 3 work committed — 7 commits landed on `Drew/Sprint3/Audit` (admin audit log viewer, username-staleness/email-claim-resolver fix, exclude `editorJson` from Audit.NET diffs, permission lockdown flip to deny-by-default post-UAT, Templates:View policy + action-tier gating, local appsettings override support, docs sync). Working tree now clean. PR #9 (TipTap editor spacing) already merged. Monday.com/EL Tasks board unreachable this run (connector down) — status there unconfirmed.
- 2026-07-21 — CDH_EL: still on `Drew/Sprint3/Audit`, Phase 3 work (9 dirty files) remains uncommitted — 5 days now (unchanged since 2026-07-16). EL Tasks board shows Drew's 3 in-progress items (audit trail, notes, application permissions) all bumped 2026-07-20T13:41 UTC, consistent with this branch's work; no new completions.
